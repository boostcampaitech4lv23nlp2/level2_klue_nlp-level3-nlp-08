...
def last_preprocessing(dataset,mode="typed_entity_marker_punct"):
  concat_entity=[]
  len_addtoken=0
  ...
  if mode=='typed_entity_marker':
    ...
    print(new_tokens)
    out_dataset = pd.DataFrame({'id':idli, 'sentence':concat_entity,'subject_entity':dataset['subject_entity'],'object_entity':dataset['object_entity'],'label':dataset['label'],})
...
def val_load_data(dataset_dir,mode="typed_entity_marker_punct"):
    pd_dataset=pd.read_csv(dataset_dir)
    if mode=="base":##사용하면 성능안좋을 확률 높다
      dataset,len_token=preprocessing_dataset(pd_dataset)
      return dataset,len_token
