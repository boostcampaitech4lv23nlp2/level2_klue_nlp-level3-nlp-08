...
class NLPAnalyzer():
    ...
    def show_group_dist_plot(self, show_col, violin_mode=False, size=(500, 1000)):
        df_list = [self.df_dict[key] for key in self.enabled_df_name_list]
        subtitle = self.enabled_df_name_list
...
def filtering(analyzer:NLPAnalyzer, target_col, query, ref_df_name=None, mode='contain'):
    """_summary_
    ref_df_name 이 있을 경위, 해당 dataframe을 필터링해서 남은 id만 데이터를 남김.
    Args:
        target_col (_type_): _description_
        query (_type_): _description_
        mode (str, optional): _description_. Defaults to 'contain'.
    """
    new_analyzer = copy.deepcopy(analyzer)
    if ref_df_name:
